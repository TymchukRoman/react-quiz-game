import { prepareStyles, prepareQuestions, prepareAnswers } from "./preparators";
import { styleValues, questions, answers } from "./test.data";


describe('Styles preparator test', () => {
    styleValues.forEach((value: { name: string, input: any, expected: any }) => {
        it(value.name, () => {
            const styles = prepareStyles(value.input);
            expect(styles).toEqual(value.expected);
        })
    })
})
